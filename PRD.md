...
**Stealth review Phase 5 — challenge interaction, approved scope (2026-09-25, unreleased).** Two changes. **(1) Turnstile click:** on Chromium, when a stealth render still carries a Cloudflare wall after the wait-out and the page has a `challenges.cloudflare.com` frame, `scrapeWithStealth` clicks the checkbox at a fixed offset into the frame's box via `page.mouse`. This is plain Playwright with no patchright, and it then waits for the token or a title change. Pure helpers are `pickTurnstileBox` (which skips off-screen widgets) and `turnstileClickPoint`. Verified on a local page with Cloudflare's forced-interactive test sitekey (no click, no token; click, `XXXX.DUMMY.TOKEN.XXXX`), which **proves the mechanism only**, not a real-site bypass. **(2) Verdict fix:** nowsecure.nl turned out not to be a wall at all. It serves 200 to the honest UA, with 43 characters and two widgets on Cloudflare's test key, and every recorded "Blocked" came from our detector. `challengeDetection.js` now clears a 200 document with a title and text, no bootstrap, no challenge wording and only widget markers. A 403 widget-only page and an empty 200 shell stay walls. Benchmark row relabelled. **Held:** the patchright spike. **Not done:** cType detection. 15 new tests (2 live, gated on Chromium plus reachability of challenges.cloudflare.com); `npm run test:unit` 2468 tests / 0 failed (184 files); MCP compliance 100.0% / 0 errors. No version bump.

**v6.10.0 released 2026-09-25** — minor release: stealth review Phase 4, below. **Stealth review Phase 4 — session persistence (2026-09-25).** A bot-wall challenge solved once is not solved again. New `src/core/ClearanceJar.js`: when a stealth context closes, the vendor clearance cookies it earned (exactly `cf_clearance`, `__cf_bm`, `datadome` — no site cookie, so no caller's login can reach another's context) are kept under a hashed key of engine + User-Agent + proxy server/username, and `createStealthContext` replays them to the next context with that identity. A render that still meets the wall discards the host. Expiry is the cookie's own, capped at 24 h; 32 identities × 200 cookies; persisted at `~/.crawlforge/stealth-clearance.json` (0600); `CRAWLFORGE_CLEARANCE_JAR=off` disables it; the stealth benchmark always runs with it off. **Gate passed on both engines:** stackoverflow.com's first stealth call went 307 → 403 → 302 → 200 through Cloudflare's `orchestrate`/`fo` challenge (4.1 s), the second went straight to 200 with neither (1.6 s on Chromium); a second process reused a clearance from disk with 0 challenge-platform requests (indeed.com). **Not built:** the persistent Chromium profile pool (`launchPersistentContext`) — a shared profile carries logins between hosted customers and breaks the shared-browser/per-context-proxy design; left for an owner decision (review §7, decision 7). 14 new unit tests; `npm run test:unit` 2453 tests / 0 failed (183 files); MCP compliance 100.0% / 0 errors. No credit or schema change; shipped as a minor (6.10.0) because it adds a feature.

**v6.9.0 released 2026-09-22** — minor release (the `agent` price changes): stealth review Phase 3, below; a stealth retry that doesn't get the page is free; and CI green again on `main` (the Unit Tests job now installs Chromium, and the `adm-zip` override moves to `^0.6.1`, clearing `npm audit`). **Phase 3 of `docs/STEALTH_REVIEW_2026-09.md`: the `agent` tool browses.** When ACT's plain fetch hits a challenge page, a 403/429, an empty shell or a timeout, the agent now retries the URL through the stealth stage `scrape`'s `escalate: true` uses. It is one `stealthEscalation` function in `server.js` shared by both tools, so the gate, `resolveStealthEngine()`, `CRAWLFORGE_STEALTH_PROXIES` and `CRAWLFORGE_STEALTH_ENGINE` all apply. The retry is automatic and capped at 2 a run (owner decision 4). URLs the caller named are retried first. A discovered URL keeps its snippet fallback when it has a relevant snippet. A named URL whose retry fails also falls back to its snippet (a second owner-approved decision). Refusals, 404s and 5xx errors are never retried. Evidence from a retry is marked `via: "stealth"`, and results gain `stealth_retries` and `warnings`. **Pricing:** 8, plus 5 per retry that gets the page (a retry blocked again is free, owner decision; `stealth_retries_charged` reports the billed count). The projection is the 18-credit ceiling (13 at `maxUrls: 1`, 8 for pro), and `setActualCost` bills what ran. New `src/tools/agent/escalation.js` and `tests/unit/agent-stealth-escalation.test.js` (9 tests). **Live gate:** passed with `CRAWLFORGE_STEALTH_ENGINE=chromium` (3.3 stars, 58,942 reviews; the page moved on from the review's 58,941; the seed was `via: "stealth"`, not a snippet). It failed on the default `'auto'`/Camoufox engine from exit IP 64.71.236.132, where the unchanged `scrape` escalation fails too. **Website parity done:** crawlforge-website `d8741be` sets `TOOL_CREDIT_COSTS.agent` to the 18 ceiling, and the REST route charges 8 + 5 × `stealth_retries_charged` (cost parity 31/0). The Phase 0 hosted-baseline box is also ticked, now that both 2026-09-22 hosted runs exist. This is a credit and behaviour change, hence the minor bump. 2437 unit tests / 0 failed (182 files), MCP compliance 100.0% / 0 errors.

**v6.8.0 released 2026-09-22** — minor release bundling the five entries below: the 2026-09 stealth review Phases 0-2 (benchmark harness, the measured fingerprint/verdict fixes, and `'auto'` engine routing with proxy plumbing), the Docker fixes that let the image run Camoufox at all, the per-deployment `CRAWLFORGE_STEALTH_ENGINE` pin the hosted baseline forced, and the creator-mode secret rotation. Minor rather than patch because callers gain an `engine` parameter on `browser_session` and `scrape_with_actions` and the stealth engine default moves to `'auto'`; `engines.node` deliberately stays `>=20.16.0` even though the Docker image moved to Node 22.

**Shipped in v6.8.0 (security)** — **creator-mode secret rotated.** `CREATOR_SECRET_HASH` in `src/core/creatorMode.js` replaced with the SHA-256 of a freshly generated secret. The local `.env` and the `crawlforge` MCP-server entry in `~/.claude.json` were updated in step — the latter matters because its explicit value overrides `.env` for the running server — and two stale Claude Code permission rules that still carried the old secret in plaintext were removed. The old secret never reached git history (`git log --all -S` clean; `.env` never tracked). Verified over `node server.js`: the new secret prints "Creator Mode Enabled", the old prints "Invalid creator secret provided", an empty value enables neither. The hash stays safe to commit — one-way, as the file notes. **v6.8.0 is the first published version carrying the new hash.** Everything published before it (npm `latest` was 6.7.0) keeps honouring the previous secret, which no publish can retract; the globally installed CLI accepts the new one only after `npm i -g crawlforge-mcp-server@6.8.0`. The hash was rotated once more before release, to a value generated by the maintainer outside the session. No tool, schema or credit changes; no version bump. 2429 unit tests / 0 failed (181 files), MCP compliance 100.0% / 0 errors.

**Shipped in v6.8.0 (stealth)** — **Phase 2 of `docs/STEALTH_REVIEW_2026-09.md`: engine routing and proxy plumbing — the engine that actually passes the walls is now the one that runs.** Shipped by five parallel sub-agents (stealth core, tool surface, research paths, deploy/Docker, documentation) and integrated, diff-verified and gate-run in the main session. **Routing:** `scrape.escalate_engine` and `stealth_mode.engine` default to `'auto'` instead of `'playwright'`/`'chromium'` — prefer Camoufox, fall back to Chromium when its binary is absent and say so in the caller's `warnings`, reusing the semantic `deep_research` already had under `RESEARCH_STEALTH_ENGINE`. One resolver, `resolveStealthEngine(requested)` → `{engine, fallbackWarning}`, backs every entry point; naming an engine still pins it exactly and `'playwright'` stays the public name for Chromium, so no existing call changes behaviour. `server.js` previously collapsed anything non-Camoufox to Chromium, which would have defeated `'auto'` outright. **Surface:** `engine` parameter added to `browser_session` and `scrape_with_actions`, carried through `BrowserProcessor` to *both* `launchStealthBrowser` and `createStealthContext` — passing it to only one relaunched on the default underneath. **Proxies:** `CRAWLFORGE_STEALTH_PROXIES` (comma-separated, server-level) is consumed in `resolveProxy()`, the single choke point for every path through `StealthBrowserManager`, plus separate wiring for `ResearchOrchestrator`, which launches Camoufox directly and bypasses the manager; a proxy passed on the call always wins, and it is kept distinct from the localization-owned `PROXY_ROTATION_*` family. `geoip: !!proxy` was **already shipped in Phase 1** and was verified rather than rebuilt. **Fixed, and the reason this phase mattered:** Camoufox's UA self-contradicted on about half of all launches — `camoufox@0.1.19` rewrites persona versions with a non-global regex, so the rewrite reaches `rv:` and stops while `Firefox/` keeps whatever browserforge drew (`rv:135.0 … Firefox/150.0`, measured 4/8 on the installed binary). Personas are now generated at the binary's own major, making the rewrite a no-op (8/8, confirmed on a real launch through the manager). The defect predates the phase; defaulting to `'auto'` is what promoted it from an opt-in path to the default one. Also: the Chrome-shaped extra-stealth init script is no longer injected into Firefox, and `headless:'virtual'` no longer collapses to plain headless via `config.headless !== false`. **Docker:** the image could never have run Camoufox — musl cannot load a glibc-linked binary, and `camoufox`'s transitive `language-tags@2.1.0` needs Node ≥22, which npm drops silently for an optional subtree (measured: `node:20-bookworm-slim` → no `node_modules/camoufox`, no error). Base moved to `node:22-bookworm-slim`; `engines.node` deliberately stays `>=20.16.0`, since Node 20 users now get Chromium *with* the warning. **Not done, and said so:** the Camoufox binary was **not** upgraded — upstream 152 is outside browserforge's data, where *every* UA self-contradicts, so a newer binary is measurably worse and the fix is a current client; the `camoufox-js` evaluation stays open. **Hosted verification is outstanding**: the phase's gate needs the Phase 0 baseline from the hosted instance and cannot be satisfied from a dev machine. Camoufox costs ~+0.8 s and ~+400 MB RSS per call, which `'auto'` now pays on the common path. No credit changes; no version bump. 2423 unit tests / 0 failed (181 files), MCP compliance 100.0% / 0 errors.

**Shipped in v6.8.0 (stealth)** — **Phase 1 of `docs/STEALTH_REVIEW_2026-09.md`: the leaks and false verdicts the Phase 0 harness measured, closed and re-measured.** Shipped by three parallel sub-agents (engine/fingerprint, verdict layer, documentation) and integrated, measured and gate-verified by the PM; every number below is from `npm run bench:stealth` on a residential connection, with the "before" produced by running the same harness against a clean HEAD worktree rather than quoting the review. **Verdict:** a page that merely embeds a Turnstile widget is no longer called a block — quora.com rendered its real login page and was reported blocked for three runs, because the page carries exactly one `cf-chl-` token and it is `cf-chl-widget-<id>_response`, the hidden input Turnstile names itself, so a bare `cf-chl-` test matches every widget embed there is; the refinement separates widget markers from the challenge bootstrap, `stealthVerdict` re-runs upstream with them neutralised so error pages and empty shells still decide, and `scrape` (which imported the verdict straight from `crawlforge-extractors`) now goes through the same wrapper. quora.com is a **Pass** on both engines; nowsecure.nl stays Blocked, the honest answer for an interactive Turnstile, and now at least gets its wait — `_waitOutChallenge` triggers on the bootstrap marker, not only on a known title. **Fingerprint:** `navigator.webdriver` reports `false` instead of being deleted and nothing is an own property of the navigator instance any more (`connection`, `plugins`, `mimeTypes`, `getBattery` moved to `Navigator.prototype`), which is what closed rebrowser's last red row; `userAgentData` carries "Google Chrome" and no headless brand; the UA's Chrome major is read from the installed binary; the persona's OS is the host's on both engines; and worker identity (UA, platform, accept-language, cores) moved from init scripts to CDP emulation, so a Worker answers what the document answers — with `hardwareConcurrency` and `deviceMemory` now observed rather than drawn, because no CDP override reaches a worker and a spoofed document beside a truthful worker was the contradiction being scored. **Flags:** `--disable-web-security` and `--disable-site-isolation-trials` removed from the stealth browser (the non-stealth `BrowserProcessor` still carries both), the four "stealth driver" defaults taken off the command line via `ignoreDefaultArgs`, the five no-op `--disable-webrtc-*` flags replaced by `--webrtc-ip-handling-policy=disable_non_proxied_udp`, a hover/pointer `--blink-settings` pair added, and `page.route('**/*')` deleted along with the advanced level's random image/font aborts. **Camoufox:** the caller's `locale` now reaches the launcher (browser-wide until `cleanup()`, `geoip` wins behind a proxy) instead of a context override that reached the document only. **Result:** detector self-probes **19 pass / 0 fail / 1 skip** on both engines; seven of the eight seeded `ci-baseline.json` ids deleted; rebrowser all green on Chromium; CreepJS worker mismatch down from 3 fields to 1. **Not closed, and said so:** CreepJS's remaining worker UA is a SharedWorker, a CDP target nothing attaches an `Emulation` override to; incolumitas' `WEBDRIVER` FAIL is an fpscanner-era test that flags every real Chrome since 89; sannysoft's two red rows fail identically on HEAD; and camoufox npm 0.1.19 silently ignores its documented `os` option (it forwards the key to `fingerprint-generator`, which wants `operatingSystems`), so `persona-os-vs-host` stays baselined for that engine and passes to Phase 2. No tool, schema or credit changes; no version bump. 2372 unit tests / 0 failed (178 files), MCP compliance 100.0% / 0 errors.

**Shipped in v6.8.0 (tooling)** — **stealth benchmark harness (Phase 0 of `docs/STEALTH_REVIEW_2026-09.md`)**. The 2026-09-21 stealth review's benchmark was run by hand, one MCP call at a time, and every later phase of that plan is supposed to be measured against it — which only works if it can be produced again. `scripts/stealth-bench.mjs` (+ `scripts/lib/stealth-bench/`) drives `StealthBrowserManager` and the plain fetch against the review's twelve bot walls and five detector pages and prints the same matrix, headed by host OS, exit-IP classification (bot.incolumitas.com's IP API), engine, browser and Playwright versions. Ten in-page self-probes encode the section 2.3 assertions (`navigator.webdriver`, `userAgentData` brands, worker-vs-main-thread UA/platform/cores/languages, WebRTC candidate leak, UA-version-vs-binary, persona-OS-vs-host, headless markers); `--ci` runs those alone — no third-party sites — and gates on `ci-baseline.json`, so the leaks the review already documented stay green and a **regression** goes red. `--self-check` forces `navigator.webdriver` to `true` and fails unless the harness catches it. New workflow `.github/workflows/stealth-detectors.yml` runs both; new npm scripts `bench:stealth` and `bench:stealth:ci`. **First real run committed as `docs/stealth-bench-baseline-2026-09-21-residential.md`**: eleven of the twelve wall rows reproduce section 2.2 (Harrods passes on Camoufox alone, stackoverflow on both engines, trustpilot robots-skipped, Quora still the finding-6 false positive). The twelfth, leboncoin on Chromium, passed on one run and was blocked on another an hour later from the same IP with no code change in between — the harness's first useful result is a demonstration that a single DataDome cell is not evidence. The run also corrected three of the review's own claims — the Chromium UA pool is randomised, not Windows-only; the spoofed UA *does* reach Web Workers (what leaks is `platform` and `hardwareConcurrency`); and WebRTC host candidates are mDNS-only, so the finding-9 IPv6 leak is neither confirmed nor refuted without a STUN-backed check. The hosted-instance baseline is **still outstanding** and deliberately not inferred. No version bump, no tool changes, no credit changes; 2322 unit tests pass, MCP compliance 100%.

**Shipped in v6.8.0 (docs)** — `CLAUDE.md` tool inventory corrected against the live registrations in `server.js`: the count was stale at 28 (5 inline + 23 advanced) and both lists were missing entries. Ground truth is **29 tools = 6 inline + 23 advanced**; `extract_embedded_state` added to the basic list and `get_batch_results` to the advanced list, which had carried only 22 names under a "(23)" heading. "Current Version" moved 5.3.0 -> 5.6.0. Docs only, no code change, no version bump.
...
**v5.3.0 released 2026-08-28** — minor release, the largest since 5.2.0: the whole 2026-08-28 tool-quality sweep plus the compliance work. **Every fetching tool now checks robots.txt and the crawler has one honest identity** (product token `CrawlForge`; the retired `CrawlForge-Bot` still honoured as a disallow), including two stealth paths that previously walked past the gate — and **a compliance refusal no longer bills the caller**. Outbound requests are signed per RFC 9421 (Web Bot Auth, Ed25519 + `Signature-Agent`). Tool fixes, each verified live over MCP stdio: `scrape` silently discarded data tables at its default setting (S&P 500 list returned 0 rows, now 503); `extract_with_llm` returned `{}` with `success: true`; `extract_structured` answered from page chrome (`headline: "Skip to content"`); `crawl_deep`’s `include_patterns` blocked its own start URL, making the parameter unusable, and its `depth_distribution` reported path depth rather than crawl depth; `track_changes` reported zero changes while the diff showed ten, so alerts never fired; `scrape_structured` returned arrays that were not row-aligned (new optional `row_selector`); `summarize_content` led with navigation chrome; `deep_research` promoted an arXiv affiliation block and a competitor’s marketing copy into its conclusions; `process_document` returned two different readability scores for one document (there were three Flesch implementations); `analyze_content` ignored the CJK segmenter in its readability block; `search_web` returned short after dedup; `generate_llms_txt` truncated in crawl order, omitting the homepage, docs and specification; `scrape` `branding` emitted `var(--default-font-family` as a font. Depends on `crawlforge-extractors` 1.2.3, which stops `scrape_template github-repo` returning GitHub’s “Contribute to …” boilerplate as the description. 1439 unit tests / 1438 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.3.1 released 2026-08-28** — patch release: **`deep_research` judges its claims with a model measured fit to judge.** Relevance, same-meaning grouping and contradiction now route to `gemma3:12b` when it is installed (the 4B extraction winner scored "Playwright vs Selenium" marketing 0.9 relevant to an anti-bot topic and put it in the summary); conflict detection is back on, gated on that measured model or a cloud provider rather than a flag, and the orchestrator's 40-pair candidate cap now matches the judge's.

**v5.2.9 released 2026-08-26** — patch release: **`analyzeRelevance` silently degraded to word-overlap scoring on every Ollama run** — the last raw `JSON.parse` in LLMManager choked on the markdown fences small local models emit, so per-source LLM relevance scoring never actually ran. Now schema-constrained + fence-stripped + validated with one retry (same pattern as the 5.2.8 synthesis fix); also fixes `|| 0.5` coercing a legitimate relevance score of 0. Live: 0/4 fallbacks (was 4/4), scores discriminate 0.95 on-topic vs 0.1 off-topic. 1178 unit tests / 1177 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.8 released 2026-08-26** — patch release: two `deep_research` leftovers from 5.2.6. **LLM synthesis fell back to the extractive stub ~2 runs in 3** — gemma overran the 800-token budget and truncated the JSON mid-string, with whole 1,500+-char findings bloating the prompt; now findings are capped at 300 chars, the prompt asks for brevity, the budget is 1,600 tokens, and one retry precedes the fallback (5/5 live syntheses after, ~2× faster). And **`researchGaps`/`consensus`/gap-filling recommendations still keyword-joined** ("have more real options than year most them good very") — all three sites now quote the group's most credible claim via shared `mostCredibleClaim()`/`claimGroupLabel()` helpers. Verified live over MCP stdio on the original repro topic. 1174 unit tests / 1173 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.7 released 2026-08-26** — patch release: **unscoped `reddit_search` failed with `-32602` over MCP stdio** — the tool's declared output schema never learned the `web_discovery` source value added in 5.2.3, so the MCP layer rejected the tool's own successful result. Scoped searches passed, and both pre-ship verification (direct `execute()`) and the REST route bypass MCP output validation, so only a live stdio call surfaced it. Enum fixed, regression test pins the rejected shape, verified end to end over stdio. 1167 unit tests / 1166 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.6 released 2026-08-26** — patch release: seven result-quality defects found by pointing the tools at live pages. Unscoped `reddit_search` **failed with a DNS error from every npm install** (the web-discovery adapter's default base URL was the unresolvable `api.crawlforge.dev`; now `www.crawlforge.dev`, passed through explicitly). `agent` answered current-state prompts ("#1 on Hacker News right now") from dated articles without ever fetching the live page — a deterministic recency gate now queries the bare entity name, domain-votes the results and promotes the dominant origin's root to the front of the fetch queue and synthesis input. `deep_research` findings were stopword-stripped keyword joins (now the most credible claim verbatim) and its LLM synthesis never reached the response (now `insights`/`aiSummary`; fenced JSON from small local models parses). CJK text was whitespace-tokenized — word counts were paragraph counts, keywords were whole sentences — now `Intl.Segmenter` dictionary segmentation. `summarize_content` led with "Jump to content" nav chrome (leading boilerplate stripped, size-guarded). `extract_metadata` concatenated inline SVG `<title>` labels into titles and missed `name=`-spelled OG tags (MDN). `process_document` accepted `extractTables` and silently discarded it, with no implementation at any layer — PDF tables are now detected from the pdfjs text layer (Transformer paper Table 2 verified cell-exact). `crawlforge-extractors` raised to `^1.2.2` for the `github-repo` template against GitHub's React code view (license/homepage/open_issues from the embedded JSON payload). All fixes live-verified (github.com, MDN, en/zh Wikipedia, arxiv.org, HN vs the Firebase ground truth, end-to-end reddit web discovery). 1166 unit tests / 1165 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.5 released 2026-08-26** — patch release: three `track_changes` defects found while testing price tracking on Zillow and Newegg. Element-level analysis indexed a fixed tag allowlist (`h1`-`h6`, `p`, `div`, `span`, `a`), so **`customSelectors` scoped to any other tag indexed zero elements** and no element-level change could ever be reported — a Zillow page scoped to `['address']` built a baseline of 0 elements from 9 matching nodes. A scoped compare also discarded the caller's tracking options in favour of the baseline's (correct, and required for a valid diff) but said nothing, so it returned results identical to an unscoped run; the ignored options are now reported through a `warnings` array. And "Text content changed" appeared on compares returning `hasChanges: false`, because sub-threshold token noise still populates `textChanges` — the summary now defers to significance. `crawlforge-extractors` raised to `^1.2.1` for the `youtube-video` view-count fix. 1140 unit tests / 1139 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.4 released 2026-08-26** — pricing change: **`reddit_search` now costs 5 credits, up from 2**, matching `search_web`. A Reddit-wide keyword search discovers posts through the same web search `search_web` runs before reading them from the Arctic Shift archive, so it spends the same upstream call; scoped searches still query the archive directly but bill identically, since the tool has one published price. Client-side cost projection, skill credit tiers and the routing notes in `CLAUDE.md` are updated to match. 1133 unit tests / 1132 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.3 released 2026-08-26** — patch release: the two remaining findings from the 28-tool live sweep. **PullPush stopped serving automated clients**, returning 429 "This website does not provide free scraping resources for agents" on every user agent (a Cloudflare 403 challenge from some IPs), and it was the only backend for a keyword search across all of Reddit — Arctic Shift rejects a keyword query that names no subreddit or author. `reddit_search` now serves a Reddit-wide search by finding posts with a site-restricted web search and reading those posts out of the archive by ID, returning real archive rows rather than search snippets; PullPush is no longer tried automatically anywhere, including as the fallback for scoped searches where it could only bury the real error. The **`npm-package` template** now reads the npm registry API (via `crawlforge-extractors` 1.2.0) instead of scraping npmjs.com, which answers plain fetches with 403 — version, license, repository, homepage, maintainers and dependencies were all null or wrong before. 1133 unit tests / 1132 pass / 1 skipped, MCP compliance 100% with 28 tools.

**v5.2.2 released 2026-08-26** — patch release: two result-quality fixes found by a live regression sweep of all 28 tools. The `markdown` format returned raw HTML for any page built on layout tables (Hacker News and plenty of older sites): turndown-plugin-gfm passes every table whose first row is not all `<th>` through its `keep` filter verbatim, so what came back was `<table>` markup in the field a caller asked for as markdown. Rules added after the plugin now reclaim those tables and flatten them to their cell content, while real data tables still render as GFM pipe tables. And `ContentAnalyzer` language detection whitelisted ISO 639-2/B codes that franc never emits, so **Chinese, Greek, Arabic, Norwegian and Malay were silently undetectable** — a page written entirely in Chinese returned no language at all. Those codes are corrected, and because franc scores the single most common script, a CJK page carrying English brand names and code samples is now settled by a Han/kana/hangul share check before trigram scoring. 1127 unit tests / 1126 pass / 1 skipped, MCP compliance 100% with 28 tools.
