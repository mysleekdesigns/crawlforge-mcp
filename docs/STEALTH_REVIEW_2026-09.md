...
| --- | --- | --- | --- | --- |
| nowsecure.nl | ~~Cloudflare, interactive challenge~~ Not a wall: a 200 page embedding a Turnstile widget on Cloudflare's test sitekey (see note) | Blocked* | Blocked* | Blocked* (also at 15 s) |
...
- nowsecure.nl: the interstitial's title is `nowsecure.nl`, which is not a known challenge title, so the challenge wait-out is skipped and the block verdict fires on the script marker. Even with a 15 s wait Camoufox did not pass it, so it is most likely an interactive Turnstile that needs a click.
  - **\* Corrected 2026-09-25 (Phase 5): the three Blocked cells came from our own detector, not from Cloudflare.** nowsecure.nl answers **200** to the honest `CrawlForge/<version>` fetch with the full 179 KB page. That page is a nodriver demo whose visible text is 43 characters ("NOWSECURE by nodriver" twice), plus two Turnstile widgets on `data-sitekey="3x00000000000000000000FF"`. That is Cloudflare's forced-interactive test key, with no callback and no server check. There was never an interstitial and no `_cf_chl_opt`; the verdict fired on the widget's script host on a short page, the finding-6 false positive below the Phase 1 200-character floor. After the Phase 5 verdict fix it reads Pass on the plain fetch and on both stealth engines. The row stays in the harness as the check that a widget-only 200 page is not flagged.
...
- [x] **quora.com is a pass**, on both engines, with the real title and its 397 characters of body text. The cause was not what the plan assumed: the page carries exactly one `cf-chl-` token and it is `cf-chl-widget-gaztz_response`, the hidden input **Turnstile names itself**, so a bare `cf-chl-` test — ours and upstream's — matches every widget embed there is. The refinement now clears the widget's own markers and keeps the bootstrap (`_cf_chl_opt`, `window._cf_chl`, `cf_chl_rc_`), and `looksLikeInterstitial` gives a custom-titled wall its wait before any verdict. nowsecure.nl is still Blocked on both engines, which is the honest answer for an interactive Turnstile: Phase 5's work, not this one's. *(Corrected in Phase 5: nowsecure.nl is a 200 page embedding a test-key widget, and that Blocked was our own verdict. See the note under 2.2.)*
...
3. **A retry that doesn't get the page is free** (owner decision, 2026-09-22). The +5 is charged only when the stealth render gets past the wall (`documentVerdict(...).success`). A retry that meets the wall again, or throws, still counts toward the cap of 2, since it launched a browser, but costs nothing. The result reports both counts: `stealth_retries` (attempts) and `stealth_retries_charged` (billed). An errored run keeps the server-wide half-price rule, and a run that finishes without an answer is charged in full. The owner chose to keep both.

**crawlforge-website parity: done** (`d8741be`, 2026-09-22). Hosted REST calls arrive as internal requests, which the MCP server does not bill; the website charges its own table. `TOOL_CREDIT_COSTS.agent` is now the 18 ceiling, reserved up front as `browser_session`'s is. `getAgentCreditCost()` charges 8 + 5 × the backend's `stealth_retries_charged`, clamped to 2. `verify-cost-parity.mjs`: 31 tools, 0 mismatches.
...
**Completed:** 2026-09-25, in the scope the owner approved: the checkbox click, verified on a local test-key page, and the nowsecure.nl verdict fix. The patchright spike is **on hold** by owner decision, and cType detection was not in scope. The verification gate was replaced (below), because the one written here targeted a page that turned out not to be a wall.
...
- [x] For managed and interactive: locate the `challenges.cloudflare.com` iframe, click a fixed offset inside its bounding box, then wait for the title change and `domcontentloaded`. — `StealthBrowserManager._clickTurnstile()`. Vanilla Playwright is enough to *locate* the frame: `page.frames()` lists the cross-origin challenge frame whatever closed shadow root it is mounted in, and `frameElement().boundingBox()` returns its box (measured on nowsecure.nl). The click is `page.mouse` at `turnstileClickPoint(box)`: 28 px in from the left edge (the checkbox centre of the 300×65 widget, clamped inside a narrower box), vertically centred. `pickTurnstileBox()` skips a widget whose click point is off screen; nowsecure.nl mounts one at y = −147. After the click it waits up to 10 s for the widget's token or a title change, then hands a wall that is now navigating to `_waitOutChallenge`. `scrapeWithStealth` calls it through `_clickThroughChallenge()`, which clicks **only on Chromium** (the approved scope; Camoufox is untested) and **only when the verdict, with the HTTP status, still calls the document a Cloudflare wall**. A real page that embeds a widget, such as Quora's login or nowsecure.nl at 200, is never clicked. Without cType detection, the trigger is the verdict rather than the challenge type, so a non-interactive challenge that is still up after the wait-out is clicked too. What that click does to such a challenge has not been measured.
- [ ] For an embedded widget in the page body: needs closed-shadow-root access. Run the patchright spike (does `addInitScript` still land where the 13 init scripts expect it) and adopt patchright only if the spike passes and Phase 0 stays green. — **On hold by owner decision (2026-09-25).** The click above does not need it, because the frame box is reachable without entering the shadow root.
- [ ] Keep the honest failure: if the challenge does not pass, the verdict still names the vendor and the next step. — Unchanged rather than re-verified: the click runs before the read, and the read still goes through the same verdict, which names the vendor (pinned for a 403 widget-only wall in `stealthPhase5.test.js`). The next-step wording was not re-checked in this phase.
...
- [x] **Gate replaced by owner decision: a local test-key page.** nowsecure.nl could not serve as the gate. It is not a wall (see the note under 2.2), and its widget uses Cloudflare's test sitekey, so no click could ever change what the page says. The replacement lives in `tests/unit/stealthPhase5.test.js`. It serves, from 127.0.0.1, a page that loads the real Turnstile script with the forced-interactive test sitekey `3x00000000000000000000FF`, on stealth Chromium. **With no click, the widget issues no token** (control, 4 s after the widget painted). **With `_clickTurnstile`, it issues `XXXX.DUMMY.TOKEN.XXXX`.** It passed on each of 4 runs, the last inside the full unit suite. Because it needs `challenges.cloudflare.com`, the live suite skips when Chromium is missing or that host is unreachable (so it skips under a sandbox). The click-point, box-selection and gating logic are covered by pure tests with no browser.

  **What this proves is the mechanism only:** the frame is found, the click lands on the checkbox, and the widget accepts it. It is **not** evidence that any real site's challenge accepts the click. A real challenge scores the IP and the fingerprint before and after the click, and the test key scores nothing. The live measurement is still outstanding, and no site in the benchmark is known to serve a challenge that does not solve itself (Phase 4 saw stackoverflow's solve itself and indeed.com's pass in the background).
- [x] **nowsecure.nl reads Pass** on the plain fetch and on both stealth engines (Chromium and Camoufox, both 200, 43 characters, residential IP, 2026-09-25). This is because of the verdict fix, not the click. `challengeDetection.js` now clears a Cloudflare hit on a document that was answered **200**, has a title and visible text, carries no challenge bootstrap and none of the interstitial's prose. Upstream's Cloudflare pattern is only the widget host and `cf-chl-*`, so such a hit can only come from a widget. The same document at **403** is still blocked, and so is an empty 200 shell carrying only the widget script (the Phase 1 decision, still pinned by `challengeVerdictPhase1.test.js`). `stealth_mode`'s `create_page` and the clearance jar's discard-on-block now pass the status to the verdict too. The benchmark row is relabelled and kept as the check that a widget-only 200 page is not flagged.
- [ ] **Detector-page regression after patchright:** not applicable, because patchright was not adopted (spike on hold).
