...
Local monitors are persisted on this machine, fire in-process while the MCP server runs (or via `monitor:run-due` from system cron for guaranteed firing), and notify by webhook — never email. Hosted monitors (`--hosted`) are registered with the CrawlForge hosted monitors API under your API key and run from CrawlForge's own scheduler whether or not anything is running here, with every check recorded and email plus signed-webhook notifications sent by CrawlForge.
...
Create a persisted local scheduled monitor (optionally with a plain-English alert goal), or with `--hosted` a monitor that CrawlForge runs for you.
...
Options:
  ...
  --goal <text>         Plain-English alert goal (LLM-judged; degrades to threshold if no LLM); local only
  --webhook <url>       Webhook URL to notify on meaningful changes
  --email <addresses>   Comma-separated notification emails (max 5); requires --hosted
  --threshold <level>   Notification threshold: minor, moderate, major, or critical (default: moderate); local only
  --cron <expr>         Optional cron expression (advanced)
  --selector <css>      CSS selector to scope monitoring
  --hosted              Register with CrawlForge's hosted monitors instead of saving locally
...
Examples:
  ...
  crawlforge monitor:create https://example.com/pricing --hosted --every 21600 --selector ".price" --email you@example.com,ops@example.com
```

With `--hosted`, nothing is created locally and the page is not fetched by this process. `--every` is converted to a cron schedule (5–60 minutes that divide 60, whole hours that divide 24, or daily; other values round to the nearest supported one and a `warnings` entry says what it became — runs must be at least 5 minutes apart); `--cron` passes through verbatim; `--selector` becomes the target selector; `--webhook` becomes the hosted webhook URL, with the signing secret generated by CrawlForge and returned in the output. `--goal` and `--threshold` add a `warnings` entry — a hosted monitor notifies on every changed, new, blocked or errored page. The output includes the hosted id, `nextRunAt`, `estimatedCreditsPerMonth` and a `dashboardUrl`. Creating a hosted monitor charges no credits; each hosted check bills 3 credits per compared target (blocked and errored targets are free).

Without `--hosted`, `--email` produces a `notificationError` pointing to `--hosted` — local monitors do not send email.

---

### monitor:list

List scheduled monitors of both kinds: local ones first (`hosted: false`), then hosted ones (`hosted: true`, with `nextDueAt`, `lastCheck` and `dashboardUrl`), plus `localCount` and `hostedCount`. If the hosted API cannot be reached (no API key, offline), the local list is still returned and `hostedError` says why.
...
Stop and remove a scheduled monitor by id — a local monitor if the id is local, otherwise the hosted monitor with that id is deleted (`hosted: true` in the output). Deleting a hosted monitor charges no credits.
...
Fire every due local scheduled monitor once and exit. Wire this into system cron for guaranteed firing even when the MCP server is not running. Hosted monitors do not need it — CrawlForge's scheduler runs them.
