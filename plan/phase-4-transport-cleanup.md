...
- [x] 🟠 **getting-started prompt is unusable: config object misinterpreted as argsSchema by the SDK** · CONFIRMED
  - `server.js:115`
...
- [x] 🟠 **Stateful streamable HTTP uses one shared transport: only one session ever; reconnect or DELETE bricks /mcp until restart** · CONFIRMED
...
- [x] 🟠 **Legacy stateless mode (--legacy-http / connectHttp) fails on every request after the first** · CONFIRMED
...
- [x] 🟡 **scrape inlines full base64 screenshot bytes into the JSON tool result despite publishing a resource URI**
  - `server.js:901`
...
- [x] 🟡 **search_web reads its API key only from env/.env, not from ~/.crawlforge/config.json — fails and half-bills users configured via setup**
...
- [x] 🟡 **Auto-setup path writes status banners to stdout in stdio MCP mode**
...
- [x] 🟡 **extract_structured elicitation is wired but never invoked, contradicting the documented behaviour**
...
- [x] 🟡 **MCP-sampling fallback is dead — SamplingClient is constructed without the MCP server**
...
- [x] 🟡 **Screenshot failures are reported as 'produced no image', discarding the real error**
...
- [x] ⚪ **projectCost reads params.maxPages for crawl_deep but the tool's schema field is max_pages — projection always uses the default**
...
- [x] ⚪ **BrowserBaseBackend.connect throws an empty Error, discarding status and response body**
  ...
  - Note (2026-08-04): error message fixed as specified; the misleading getBrowserBackend() comment was replaced with an accurate "defined but unwired" note rather than wiring or removing the backend — wiring it is a product decision deferred to Phase 6.
- [x] ⚪ **HMAC signature covers only event.payload, not the delivered body — standard verification always fails**
...
- [x] ⚪ **retryableStatusCodes on the webhook RetryManager never matches — HTTP 5xx errors carry no .response**
...
- [x] ⚪ **Public discovery metadata is stale: server-card advertises '20 tools' and version 3.5.1 on a 27-tool v4.10.0 server**
...
- [x] ⚪ **Insufficient-credits refusal is returned without isError:true**
...
- [x] ⚪ **result.metadata.finalUrl is always undefined (reads wrong property path)**
...
- [x] ⚪ **Recordings made with captureIntermediateStates save unreplayable executeJavaScript entries**
...
- [x] ⚪ **No coverage for the failure modes found above: prompt retrieval, second HTTP session/request, credit-check-throw billing**
...
- [x] ⚪ **No tests exercise the real browser lifecycle, webhook SSRF/timeout, or async get_batch_results paths**
...
## Verification gate
- [x] `streamableHttp` supports a **second** concurrent session, a client **reconnect** after disconnect, and a **DELETE** followed by a fresh `initialize` — each succeeds (per-session transport map).
- [x] Legacy stateless mode either serves every request (fresh transport per request) or is removed; no unhandled rejection, no hung response.
- [x] `prompts/list` advertises `getting-started` with a description and no bogus required arg; `prompts/get` retrieves it.
- [x] Nothing but JSON-RPC reaches stdout in stdio mode even on first-launch auto-setup (assert in `stdout-hygiene.test.js`).
- [x] Discovery metadata reports the real tool count (27) and version (4.10.0).
